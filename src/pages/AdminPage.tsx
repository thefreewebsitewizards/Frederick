...
import { deleteStoreProduct, ensureStoreAdminClaims, getStoreAdminEmail, isFirebaseConfigured, signInFirebaseAdmin, signOutFirebaseAdmin, useStoreOrders, useStoreProducts, type Product } from "../data/products"
          ...
          <div>
            <div className="h-20 flex items-center px-6 border-b border-white/10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-white text-base font-bold leading-tight tracking-wide">Black Eagle Bows</h1>
            ...
            <nav className="flex flex-col gap-1 p-4">
              <button
                className={`flex items-center gap-3 px-3 py-3 rounded-lg shadow-sm relative overflow-hidden group ${
                  activeTab === "inventory"
                ...
                type="button"
              >
                {activeTab === "inventory" ? <div className="absolute inset-y-0 left-0 w-1 bg-primary rounded-r"></div> : null}
                <span className={`material-symbols-outlined ${activeTab === "inventory" ? "text-primary fill-current" : ""}`}>
                  inventory_2
                </span>
              ...
              <button
                ...
                type="button"
              >
                {activeTab === "orders" ? <div className="absolute inset-y-0 left-0 w-1 bg-primary rounded-r"></div> : null}
                <span className={`material-symbols-outlined ${activeTab === "orders" ? "text-primary fill-current" : ""}`}>
                  receipt_long
                </span>
          ...
          <div className="p-4 border-t border-white/10">
            <a className="flex items-center gap-3 px-3 py-3 rounded-lg text-stone-300 hover:bg-white/5 hover:text-white transition-colors" href="#">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate">Freddie G.</p>
          ...
          <header className="flex-shrink-0 px-8 py-8 flex flex-wrap justify-between items-end gap-4">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm max-w-xl">
                {activeTab === "orders"
                  ? "Review paid orders and see the exact build details selected at checkout."
                  : "Manage your handcrafted inventory, track stock levels, and organize product listings."}
              </p>
            </div>
                      ...
                      <tbody className="divide-y divide-stone-200 dark:divide-slate-700">
                        {paidOrders.length > 0 ? (
                          paidOrders.map((order) => {
                            ...
                            const shippingCents = order.shipping?.markedUpAmountCents ?? 0
                            const total = itemsTotal + shippingCents / 100
                            const placed = order.createdAt ? new Date(order.createdAt).toLocaleString() : "—"
                            return (
                              <tr className="hover:bg-white dark:hover:bg-slate-700/50 transition-colors group" key={order.id}>
                                <td className="px-6 py-4">
                                  <div className="flex flex-col">
                                    <span className="text-sm font-bold text-slate-800 dark:text-white">#{order.id.slice(0, 8)}</span>
                                    <span className="text-xs text-stone-500 dark:text-slate-400">ID: {order.id}</span>
                                  </div>
                                </td>
                                <td className="px-6 py-4">
                                  <div className="flex flex-col">
                                    <span className="text-sm font-semibold text-slate-800 dark:text-white">{order.customer?.name ?? "—"}</span>
                                    <span className="text-xs text-stone-500 dark:text-slate-400">{order.customer?.email ?? ""}</span>
                                  </div>
                                </td>
                                <td className="px-6 py-4">
                                  <div className="flex flex-col gap-2">
                                    {order.items.map((item) => (
                                      <div className="text-sm text-slate-700 dark:text-slate-200" key={`${item.name}-${item.option ?? ""}`}>
                                        <span className="font-semibold">{item.name}</span> × {item.quantity}
                                        {item.option ? <span className="block text-xs text-stone-500">{item.option}</span> : null}
                                        {item.note ? <span className="block text-xs text-stone-400">{item.note}</span> : null}
                                      </div>
                                    ))}
                                  </div>
                                </td>
                                <td className="px-6 py-4 text-right">
                                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">${total.toFixed(2)}</span>
                                </td>
                                ...
                                <td className="px-6 py-4 text-sm text-stone-500 dark:text-slate-400">{placed}</td>
                              </tr>
                            )
                          })
                        ) : (
